 <section class="services sec-pd theme-bg">
    <div class="container">
      <div class="row">

        <div class="col-sm-12 col-md-6">
          <div class="sec-title wow animate__animated animate__bounceInLeft d-flex align-items-center gap-2 mb-2">
            <hr class="line" style="width: 40px;">
            <h4>Services</h4>
            <hr class="line" style="width: 40px;">
          </div>
          <h2 class="sec-hd wow animate__animated animate__fadeInUp mb-md-3 mb-sm-2">We're A Creative Digital Studio.
          </h2>
        </div>

        <div class="col-sm-12 col-md-6">
          <div class="d-flex flex-column align-items-end justify-content-center">
            <p class="service-para para-main wow animate__animated animate__fadeInUp text-end">Lorem ipsum dolor sit
              amet, consectetur adipiscing elit. Ut elit
              tellus,
              luctus nec
              ullamcorper mattis, pulvinar dapibus leo.</p>
            <a href="#" class="theme-btn wow animate__animated animate__jackInTheBox mt-1">Get In Touch <span class="icon"><i
                class="fa-solid fa-arrow-right"></i></span></a>
          </div>
        </div>

        <div class="col-sm-12 col-md-4 mt-4">
          <div class="card card-border-y bg-black p-4 rounded-3 wow animate__animated animate__bounceInRight">
            <div class="card-head d-flex justify-content-between">
              <h5 class="card-title wow animate__animated animate__fadeInUp">Website<br> Development</h5>
              <h5 style="word-break: keep-all" class="number">01</h5>
            </div>
            <div class="card-body px-0">
              <p class="para-main wow animate__animated animate__fadeInUp">Lorem ipsum dolor sit amet, consectetur Ut
                elit tellus adipiscing elit. Ut elit
                tellus, luctus nec.</p>
              <div
                class="bg-box mt-4 overflow-hidden d-flex flex-column justify-content-end align-items-center rounded-3">
                <i class="hover-icon circle-fill-icon fa-solid fs-4 fa-arrow-right position-absolute"></i>
                <svg class="shape" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1000 100" preserveAspectRatio="none">
                  <path class="elementor-shape-fill"
                    d="M806,94.7C619.5,90,500,20.3,500,1.7c-1,18.6-117.5,88.3-306,93C92,97.2,0,97.9,0,97.9v-0l0,0v2.3h1000v-2.3 C1000,97.7,920.3,97.6,806,94.7z M350,65.1L350,65.1L350,65.1L350,65.1z">
                  </path>
                </svg>
              </div>
            </div>
          </div>
        </div>
        <div class="col-sm-12 col-md-4 mt-4">
          <div class="card card-border-y bg-black p-4 rounded-3 wow animate__animated animate__bounceInUp">
            <div class="card-head d-flex justify-content-between">
              <h5 class="card-title wow animate__animated animate__fadeInUp">Logo<br> Design</h5>
              <h5 style="word-break: keep-all" class="number">02</h5>
            </div>
            <div class="card-body px-0">
              <p class="para-main wow animate__animated animate__fadeInUp">Lorem ipsum dolor sit amet, consectetur Ut
                elit tellus adipiscing elit. Ut elit
                tellus, luctus nec.</p>
              <div
                class="bg-box mt-4 overflow-hidden d-flex flex-column justify-content-end align-items-center rounded-3">
                <i class="hover-icon circle-fill-icon fa-solid fs-4 fa-arrow-right position-absolute"></i>
                <svg class="shape" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1000 100" preserveAspectRatio="none">
                  <path class="elementor-shape-fill"
                    d="M806,94.7C619.5,90,500,20.3,500,1.7c-1,18.6-117.5,88.3-306,93C92,97.2,0,97.9,0,97.9v-0l0,0v2.3h1000v-2.3 C1000,97.7,920.3,97.6,806,94.7z M350,65.1L350,65.1L350,65.1L350,65.1z">
                  </path>
                </svg>
              </div>
            </div>
          </div>
        </div>
        <div class="col-sm-12 col-md-4 mt-4">
          <div class="card card-border-y bg-black p-4 rounded-3 wow animate__animated animate__bounceInLeft">
            <div class="card-head d-flex justify-content-between">
              <h5 class="card-title wow animate__animated animate__fadeInUp">Ecommerce<br> Solutions</h5>
              <h5 style="word-break: keep-all" class="number">03</h5>
            </div>
            <div class="card-body px-0">
              <p class="para-main wow animate__animated animate__fadeInUp">Lorem ipsum dolor sit amet, consectetur Ut
                elit tellus adipiscing elit. Ut elit
                tellus, luctus nec.</p>
              <div
                class="bg-box mt-4 overflow-hidden d-flex flex-column justify-content-end align-items-center rounded-3">
                <i class="hover-icon circle-fill-icon fa-solid fs-4 fa-arrow-right position-absolute"></i>
                <svg class="shape" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1000 100" preserveAspectRatio="none">
                  <path class="elementor-shape-fill"
                    d="M806,94.7C619.5,90,500,20.3,500,1.7c-1,18.6-117.5,88.3-306,93C92,97.2,0,97.9,0,97.9v-0l0,0v2.3h1000v-2.3 C1000,97.7,920.3,97.6,806,94.7z M350,65.1L350,65.1L350,65.1L350,65.1z">
                  </path>
                </svg>
              </div>
            </div>
          </div>
        </div>
        <div class="col-sm-12 col-md-4 mt-4">
          <div class="card card-border-y bg-black p-4 rounded-3 wow animate__animated animate__bounceInRight">
            <div class="card-head d-flex justify-content-between">
              <h5 class="card-title wow animate__animated animate__fadeInUp">Illustration</h5>
              <h5 style="word-break: keep-all" class="number">04</h5>
            </div>
            <div class="card-body px-0">
              <p class="para-main wow animate__animated animate__fadeInUp">Lorem ipsum dolor sit amet, consectetur Ut
                elit tellus adipiscing elit. Ut elit
                tellus, luctus nec.</p>
              <div
                class="bg-box mt-4 overflow-hidden d-flex flex-column justify-content-end align-items-center rounded-3">
                <i class="hover-icon circle-fill-icon fa-solid fs-4 fa-arrow-right position-absolute"></i>
                <svg class="shape" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1000 100" preserveAspectRatio="none">
                  <path class="elementor-shape-fill"
                    d="M806,94.7C619.5,90,500,20.3,500,1.7c-1,18.6-117.5,88.3-306,93C92,97.2,0,97.9,0,97.9v-0l0,0v2.3h1000v-2.3 C1000,97.7,920.3,97.6,806,94.7z M350,65.1L350,65.1L350,65.1L350,65.1z">
                  </path>
                </svg>
              </div>
            </div>
          </div>
        </div>
        <div class="col-sm-12 col-md-4 mt-4">
          <div class="card card-border-y bg-black p-4 rounded-3 wow animate__animated animate__bounceInUp">
            <div class="card-head d-flex justify-content-between">
              <h5 class="card-title wow animate__animated animate__fadeInUp">Marketing<br> Collateral</h5>
              <h5 style="word-break: keep-all" class="number">05</h5>
            </div>
            <div class="card-body px-0">
              <p class="para-main wow animate__animated animate__fadeInUp">Lorem ipsum dolor sit amet, consectetur Ut
                elit tellus adipiscing elit. Ut elit
                tellus, luctus nec.</p>
              <div
                class="bg-box mt-4 overflow-hidden d-flex flex-column justify-content-end align-items-center rounded-3">
                <i class="hover-icon circle-fill-icon fa-solid fs-4 fa-arrow-right position-absolute"></i>
                <svg class="shape" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1000 100" preserveAspectRatio="none">
                  <path class="elementor-shape-fill"
                    d="M806,94.7C619.5,90,500,20.3,500,1.7c-1,18.6-117.5,88.3-306,93C92,97.2,0,97.9,0,97.9v-0l0,0v2.3h1000v-2.3 C1000,97.7,920.3,97.6,806,94.7z M350,65.1L350,65.1L350,65.1L350,65.1z">
                  </path>
                </svg>
              </div>
            </div>
          </div>
        </div>
        <div class="col-sm-12 col-md-4 mt-4">
          <div class="card card-border-y bg-black p-4 rounded-3 wow animate__animated animate__bounceInLeft">
            <div class="card-head d-flex justify-content-between">
              <h5 class="card-title wow animate__animated animate__fadeInUp">Mobile<br> Apps</h5>
              <h5 style="word-break: keep-all" class="number">06</h5>
            </div>
            <div class="card-body px-0">
              <p class="para-main wow animate__animated animate__fadeInUp">Lorem ipsum dolor sit amet, consectetur Ut
                elit tellus adipiscing elit. Ut elit
                tellus, luctus nec.</p>
              <div
                class="bg-box mt-4 overflow-hidden d-flex flex-column justify-content-end align-items-center rounded-3">
                <i class="hover-icon circle-fill-icon fa-solid fs-4 fa-arrow-right position-absolute"></i>
                <svg class="shape" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1000 100" preserveAspectRatio="none">
                  <path class="elementor-shape-fill"
                    d="M806,94.7C619.5,90,500,20.3,500,1.7c-1,18.6-117.5,88.3-306,93C92,97.2,0,97.9,0,97.9v-0l0,0v2.3h1000v-2.3 C1000,97.7,920.3,97.6,806,94.7z M350,65.1L350,65.1L350,65.1L350,65.1z">
                  </path>
                </svg>
              </div>
            </div>
          </div>
        </div>
        <div class="col-sm-12 col-md-4 mt-4">
          <div class="card card-border-y bg-black p-4 rounded-3 wow animate__animated animate__bounceInRight">
            <div class="card-head d-flex justify-content-between">
              <h5 class="card-title wow animate__animated animate__fadeInUp">SEO<br> Services</h5>
              <h5 style="word-break: keep-all" class="number">07</h5>
            </div>
            <div class="card-body px-0">
              <p class="para-main wow animate__animated animate__fadeInUp">Lorem ipsum dolor sit amet, consectetur Ut
                elit tellus adipiscing elit. Ut elit
                tellus, luctus nec.</p>
              <div
                class="bg-box mt-4 overflow-hidden d-flex flex-column justify-content-end align-items-center rounded-3">
                <i class="hover-icon circle-fill-icon fa-solid fs-4 fa-arrow-right position-absolute"></i>
                <svg class="shape" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1000 100" preserveAspectRatio="none">
                  <path class="elementor-shape-fill"
                    d="M806,94.7C619.5,90,500,20.3,500,1.7c-1,18.6-117.5,88.3-306,93C92,97.2,0,97.9,0,97.9v-0l0,0v2.3h1000v-2.3 C1000,97.7,920.3,97.6,806,94.7z M350,65.1L350,65.1L350,65.1L350,65.1z">
                  </path>
                </svg>
              </div>
            </div>
          </div>
        </div>
        <div class="col-sm-12 col-md-4 mt-4">
          <div class="card card-border-y bg-black p-4 rounded-3 wow animate__animated animate__bounceInUp">
            <div class="card-head d-flex justify-content-between">
              <h5 class="card-title wow animate__animated animate__fadeInUp">Digital<br> Marketing</h5>
              <h5 style="word-break: keep-all" class="number">08</h5>
            </div>
            <div class="card-body px-0">
              <p class="para-main wow animate__animated animate__fadeInUp">Lorem ipsum dolor sit amet, consectetur Ut
                elit tellus adipiscing elit. Ut elit
                tellus, luctus nec.</p>
              <div
                class="bg-box mt-4 overflow-hidden d-flex flex-column justify-content-end align-items-center rounded-3">
                <i class="hover-icon circle-fill-icon fa-solid fs-4 fa-arrow-right position-absolute"></i>
                <svg class="shape" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1000 100" preserveAspectRatio="none">
                  <path class="elementor-shape-fill"
                    d="M806,94.7C619.5,90,500,20.3,500,1.7c-1,18.6-117.5,88.3-306,93C92,97.2,0,97.9,0,97.9v-0l0,0v2.3h1000v-2.3 C1000,97.7,920.3,97.6,806,94.7z M350,65.1L350,65.1L350,65.1L350,65.1z">
                  </path>
                </svg>
              </div>
            </div>
          </div>
        </div>
        <div class="col-sm-12 col-md-4 mt-4">
          <div class="card card-border-y bg-black p-4 rounded-3 wow animate__animated animate__bounceInLeft">
            <div class="card-head d-flex justify-content-between">
              <h5 class="card-title wow animate__animated animate__fadeInUp">Creative<br> Copywriting</h5>
              <h5 style="word-break: keep-all" class="number">09</h5>
            </div>
            <div class="card-body px-0">
              <p class="para-main wow animate__animated animate__fadeInUp">Lorem ipsum dolor sit amet, consectetur Ut
                elit tellus adipiscing elit. Ut elit
                tellus, luctus nec.</p>
              <div
                class="bg-box mt-4 overflow-hidden d-flex flex-column justify-content-end align-items-center rounded-3">
                <i class="hover-icon circle-fill-icon fa-solid fs-4 fa-arrow-right position-absolute"></i>
                <svg class="shape" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1000 100" preserveAspectRatio="none">
                  <path
                    d="M806,94.7C619.5,90,500,20.3,500,1.7c-1,18.6-117.5,88.3-306,93C92,97.2,0,97.9,0,97.9v-0l0,0v2.3h1000v-2.3 C1000,97.7,920.3,97.6,806,94.7z M350,65.1L350,65.1L350,65.1L350,65.1z">
                  </path>
                </svg>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </section>